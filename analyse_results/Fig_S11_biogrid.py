...
for fold in range(1,2):
    if fold == 1:
        test_neg = np.load(f"../results/ProteinMAE_DL/{datasets[0]}_test_neg_fold{fold}.npy")
        test_pos = np.load(f"../results/ProteinMAE_DL/{datasets[0]}_test_pos_fold{fold}.npy")
    else:
        test_neg = np.concatenate((test_neg, np.load(f"../results/ProteinMAE_DL/{datasets[0]}_test_neg_fold{fold}.npy")))
        test_pos = np.concatenate((test_pos, np.load(f"../results/ProteinMAE_DL/{datasets[0]}_test_pos_fold{fold}.npy")))
biogrid_scores = []
...
exit()
plt.figure(figsize=(6, 6))
plt.boxplot([biogrid_scores, test_neg], tick_labels=["BioGRID", "Background"])
# add a horizontal line at the top 5% of the test_neg
plt.axhline(np.percentile(test_neg, 95), color='red', linestyle='--', label="Top 5% of background")
# and also one at the top scorerer of the test_neg
plt.axhline(np.max(test_neg), color='blue', linestyle='--', label="Top scorer of background")
plt.title("Boxplot of BioGRID scores, positives and negatives")
plt.ylabel("Prediction scores using ProtienMAE + PPDL")
plt.savefig("plots/biogrid_boxplot_DL_2.png", dpi=300)
...
biogrid_dict = {k: v for k, v in sorted(biogrid_dict.items(), key=lambda item: item[1], reverse=True)}
# save to tsv
with open("biogrid_scores_2.tsv", "w") as f:
