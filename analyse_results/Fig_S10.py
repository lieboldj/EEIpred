...
for entry, q in zip(all_p_tests, q_vals):
    entry["q_value"] = q
output_file = "RRI_DL_corrected_compact.csv"
method_names = ["dMaSIF", "PInet", "GLINTER", "ProteinMAE"]
        ...
        for k, alpha in enumerate(alphas):
            ...
            for entry in all_p_tests:
                if entry["method1"] == f"PPDL {method}" and entry["method2"] == f"RRI {method}" and entry["metric"] == metric and entry["alpha"] == alpha:
                    ...
                    break
                if entry["method1"] == f"RRI {method}" and entry["method2"] == f"PPDL {method}" and entry["metric"] == metric and entry["alpha"] == alpha:
                    q_value = entry.get("q_value", None)
                    mean_d = entry["mean_diff"]
                    break
            if j < 4 : 
...
plt.ylabel("Performance metrics", labelpad=5)
plt.tight_layout()
plt.savefig("RRI_DL_q-values.png", dpi=600)
