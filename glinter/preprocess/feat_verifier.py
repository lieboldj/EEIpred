...
def check_consistency(
    ...
    if repo is not None:
        ...
        if augment:
            mname = f'{lig}:{rec}'
            # remove assert, because we can have two times the same protein
...
if __name__ == '__main__':
    ...
    for mname in list(models.keys()):
        ...
        if ':' in dname:
            ...
            rec, lig = dname, dname
        # ignore the case where rec and lig are the same and handle it normal
        '''
        if rec == lig:
            print(rec, lig, dpaths)
