        ...
        try:
            if sampled_labels is not None:
                ...
                for index, item in enumerate(sampled_labels):
                    # skip samples in batch with 0 interactions
                    if torch.sum(item) == 0:
                        continue
                    roc_auc.append(
                        roc_auc_score(
...
if __name__ == "__main__":
    ...
    valset = Protein_search(phase='val', rot_aug = False, sample_type = 'uniform', sample_num = args.downsample_points, ds_type=args.ds_type, fold=args.fold)
    ...
    for i in range(starting_epoch, args.n_epochs):
        ...
        if val_loss < best_loss:  # Additional saves
            ...
            torch.save(
                ...
                model_path + "_" + args.ds_type + "_" + str(args.fold) + "_epoch{}".format(i),
            )

            best_loss = val_loss
    # remove tmp files
    os.remove(f"train{args.fold}_{args.ds_type}_tmp.txt")
    os.remove(f"val{args.fold}_{args.ds_type}_tmp.txt")
    os.remove(f"test{args.fold}_{args.ds_type}_tmp.txt")
