...
# dataset for saving results within their exon pairs. 
class Protein_search_exon(Dataset):
    def __init__(self, data_root = '../../data_collection/processed_dmasif/pairs/', K = 16,
                 phase = 'train',  rot_aug = True, sample_type = 'uniform', sample_num = 2048, ds_type = "EXAMPLE", fold = 1):
        # dataset parameters
        assert phase in ['train', 'test', 'val']
        assert sample_type in ['radius', 'knn', 'uniform']

        self.root = os.path.join(data_root) # we save all preproccse pairs in one folder
        self.phase = phase
        ...
        self.factor = 1
        self.K = K
        file_list = f"../../data_collection/cv_splits/{ds_type}/{phase}{fold}.txt".format(ds_type=ds_type, fold=fold, phase=phase)
        with open(file_list) as f:
            files = f.read().splitlines()
        info_list = f"../../data_collection/cv_splits/{ds_type}/{phase}_info{fold}.txt".format(ds_type=ds_type, fold=fold, phase=phase)
    ...
    def __getitem__(self, index):
        ...
        return xyz1, normal1, label1, curvature1, dists1, atom_type_sel1, xyz2, normal2, label2, curvature2, dists2, atom_type_sel2, self.files[index], self.info[index]
...
class Protein_search(Dataset):
    def __init__(self, data_root = '../../data_collection/processed_dmasif/pairs/', K = 16,
        ...
        self.factor = 1
        self.K = K
        file_list = f"{phase}{fold}_{ds_type}_tmp.txt".format(fold=fold, phase=phase)
        with open(file_list) as f:
            files = f.read().splitlines()
        #for _, _, files in os.walk(self.root):
        #    break
