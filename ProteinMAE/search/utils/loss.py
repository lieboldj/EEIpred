...
def compute_loss(descs1, descs2, xyz1, xyz2, label1, label2):
    ...
    for i in range(len(label1)):
        ...
        # only get loss if there is an interaction
        if torch.sum(label1[i]) and torch.sum(label2[i]):   
    ...
    return loss/return_pairs, preds_concates, labels_concates