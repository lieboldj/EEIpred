...
parser.add_argument('--origin_path', type=str, default='EEIP/', help='origin path')
...
parser.add_argument('--dataset_name', type=str, default='Test_set/', help='Dataset name')
...
parser.add_argument('--pdb_path', type=str, default='pdb/', help='pdb path to pdb files')
...
dataset = args.data_short_name
list_name = args.dataset_name

if dataset == "EPPIC":
    list_name = "EPPIC_EEIN/"
if dataset == "PISA":
    list_name = "PISA_EEIN_0.5/"
...
with open(f"{origin_path}/lists/{list_name}{mode}.txt", "r") as f:
...
all_exon = 0
counter = 0
out_dir = origin_path + f"/results/{dataset}/fold_{fold+1}/{mode}"
...
for i, pdbs in enumerate(tqdm(pdb_list)):
    ...
    PATH=origin_path + f'/old/seg/seg_model_protein_{dataset}_{fold}_{epoch}.pth'
