...
class ShapeNetDataset3(data.Dataset):
    def __init__(self,
        ...
        self.meta = {}
        splitfile = os.path.join(self.root, 'split_0',
        ...
        for file in filelist:
            ...
            if category in self.cat.values():
                if uuid[-2] != '-':
                    self.meta[self.id2cat[category]].append((os.path.join(self.root, category, 'points', uuid[:-2] + '.pts'),
...
class ShapeNetDataset3aug(data.Dataset):
    def __init__(self,
        ...
        if fold=='':
            ...
            splitfile = os.path.join(self.root, f'train_test_split'+dataset_name+'/tts'+fold,
                                     'shuffled_{}_file_list_{}.json'.format(split, class_choice[0][0]))
        filelist = json.load(open(splitfile, 'r'))
        for item in self.cat:
            self.meta[item] = []
        ...
        for file in filelist:
            _, category, old, uuid = file.split('/')
            if category in self.cat.values():
                if category=="rf":
                    if (len(uuid)==6):
                        self.meta[self.id2cat[category]].append((os.path.join(self.root, category, 'points', uuid[:4] + uuid[6:8] + '.pts'),
                                                             os.path.join(self.root, category, 'points_label',
                                                                          uuid[:-4] + '.seg')))
                ...
                if category=="lf":
                    if (len(uuid)==6):
                        self.meta[self.id2cat[category]].append((os.path.join(self.root, category, 'points', uuid[:6] + '.pts'),
                                                             os.path.join(self.root, category, 'points_label',
                                                                          uuid[:-4] + '.seg')))
                    #elif (len(uuid)==7)&(int(uuid[-1])>5):
                    #    continue
